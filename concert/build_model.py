...
OTHER = detail_sheet('Прочие',
    ...
    [('Комиссия билетного оператора',    0, 'компл.',   40000, 'ВАЖНО: если оператор удерживает её из кассы, она относится к этапу 1 — скажите, и перенесу'),
...
P['rao'] = line('3. Отчисления в РАО 8%', f'=-{G}*{V["rao"]}',
                note='Ставка 8% по вашим условиям. Базовый тариф РАО для концертов обычно 5% — стоит сверить с договором')
P['tech'] = line('4. Технические расходы: аренда звука и обеспечение площадки',
...
P['profit'] = line('5. ПРИБЫЛЬ',
                   f'={G}+{P["usn"]}+{P["rao"]}+{P["tech"]}+{P["fee_pre"]}+{P["mkt_pre"]}+{P["oth_pre"]}',
...
b1 = grid(T1, 'ПРИБЫЛЬ ДО РАЗДЕЛА С ПЛОЩАДКОЙ (этап 1), ₽',
    lambda r, col, top, i:
        f'=({CAP}*$A{r}*{PS}*{col}${top})*(1-{V["usn"]}-{V["rao"]})-{P["pre"]}')
T2 = b1 + 3
